from src.utils import a, Q
from src.constants import T1, T2
import numpy as np
...
# a1 = 8  # Fixed acceleration at gate  
...
df = np.array([[y0], [v0], [a0], [j0], [y2], [v2], [a2], [j2], [y1], [y1], [v1], [v1], [j1], [j1], [0]])

# construct the A matrix
zeros_block = np.zeros((1, 8))
A = np.block([
    [a(0,0), zeros_block], # y0
    [a(1,0), zeros_block], # v0
    [a(2,0), zeros_block], # a0
    [a(3,0), zeros_block], # j0
    [zeros_block, a(0,T2)], # y2
    [zeros_block, a(1,T2)], # v2
    [zeros_block, a(2,T2)], # a2
    [zeros_block, a(3,T2)], # j2
    [a(0,T1), zeros_block], # y1 (end of seg 1)
    [zeros_block, a(0,0)], # y1 (start of seg 2)
...
if print_dp_star:
    print("Optimal free variables (dp_star) for Y trajectory:")
    labels = [
        "v1 (velocity at y1)", 
        "a1 (acceleration at y1)", 
        "j1 (jerk at y1)"
    ]
...
print_py = False  # Set this to False to suppress printing

if print_py:
    print("Solution vector py (all polynomial coefficients for Y):")
...
if print_py:
    ...
    print("Polynomial 2 coefficients (p2_y):")
...
if save_coeffs_to_file:
    ...
    with open("data/y_trajectory_coeffs.json", "w") as f:
